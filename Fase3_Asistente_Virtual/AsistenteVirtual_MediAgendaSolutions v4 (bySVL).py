...
def buscar_medicos_especialidades(identificador, tipo="especialidad"):
    """
...
def verificar_disponibilidad(fecha, hora, identificador, tipo="especialidad"):
    ...
    if tipo == "medico":
        ...
        especialidad = buscar_medicos_especialidades(identificador, tipo="medico")
        if not especialidad:
    ...
    all_rows = [row for row in agenda_worksheet.get_all_values() if row[7].lower() != 'cancelada']
    ...
    if hora:
        ...
        if not (hora_inicio <= datetime.strptime(hora, '%H:%M').time() <= hora_fin):
        ...
        for cita in citas_reservadas:
            if cita[2] == hora:
                ...
                hora_actual = datetime.strptime(hora, '%H:%M') + duracion_cita
    ...
    horas_disponibles = []
    hora_actual = datetime.combine(datetime.strptime(fecha, '%Y-%m-%d').date(), hora_inicio)
    while hora_actual.time() < hora_fin:
        hora_actual_str = hora_actual.strftime('%H:%M')
        if (especialidad, fecha, hora_actual_str) not in citas_reservadas:
...
def agendar_cita(params):
    ...
    if tipo == "especialidad":
        ...
        medico = buscar_medicos_especialidades(especialidad, tipo="especialidad")
        ...
        especialidad = buscar_medicos_especialidades(medico, tipo="medico")
        if not especialidad:
    ...
    agenda_worksheet.update(f'A{first_empty_row}:I{first_empty_row}', cita_data, value_input_option='USER_ENTERED')
...
def buscar_cita_a_cancelar(params):
    """
    Elimina una cita agendada del usuario, requiere confirmación si no se proporciona.
    ...
    :param confirmacion: Booleano que indica si la cancelación ha sido confirmada.
    :return: Mensaje de confirmación o error.
    """
    ...
    if cita_id:
        ...
        for idx, row in enumerate(all_rows):
            if row[0] == str(cita_id) and row[4] == dni:
                ...
                original_row = agenda_worksheet.get_all_values().index(row) + 1
                # Solicitar confirmación al usuario
                return f"¿{nombre_paciente} (DNI: {dni}) está seguro de que desea cancelar la cita con ID {cita_id}, Médico {row[1]}, Especialidad: {row[2]}, el {fecha} a las {hora}? (SI/NO): "
        ...
        # Buscar y cancelar la cita por los datos del paciente en las filas no canceladas
        for idx, row in enumerate(all_rows):
...
def confirmar_cita_a_cancelar(params):
    """
    Elimina una cita agendada del usuario, requiere confirmación si no se proporciona.
    ...
    fecha = params.get('fecha')
    hora = params.get('hora')
    confirmacion = params.get('confirmacion')
        ...
        for idx, row in enumerate(all_rows):
            if cita_id:
                if row[0] == str(cita_id) and row[4] == dni:
                    ...
                    return f"Cita de {nombre_paciente} (DNI: {dni}) con ID {cita_id} cancelada con éxito."
            else:
                if row[4] == dni and row[5] == fecha and row[6] == hora:
                    original_row = agenda_worksheet.get_all_values().index(row) + 1
                    agenda_worksheet.update_cell(original_row, 8, 'Cancelada')
                    return f"Cita de {nombre_paciente} (DNI: {dni}) el {fecha} a las {hora}  cancelada con éxito."
    else:
        print("La cita no ha sido cancelada.")  
...
def handle_message(update: Update, context: CallbackContext):
    ...
    if run.status == 'requires_action':
        ...
        elif name == "buscar_cita_a_cancelar":
            result = buscar_cita_a_cancelar(arguments)
        
        elif name == "confirmar_cita_a_cancelar":
            result = confirmar_cita_a_cancelar(arguments)
...
def handle_confirmation(update: Update, context: CallbackContext):
    ...
    if pending_cancellation is not None:
        if text == 'SI':  # Si el usuario confirma la cancelación
            ...
            context.bot.send_message(chat_id=telegram_id, text="Cancelación de cita no confirmada.")
        
        # Limpiar la acción pendiente después de manejar la confirmación
        pending_cancellation = None
    else:
        context.bot.send_message(chat_id=telegram_id, text="No hay ninguna acción pendiente que confirmar.")
...
def enviar_confirmacion_cancelacion(update: Update, context: CallbackContext, params):
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    context.bot.send_message(chat_id=update.message.chat_id, text="¿Estás seguro de que deseas cancelar esta cita?", reply_markup=reply_markup)
...
def handle_callback_query(update: Update, context: CallbackContext):
    ...
    if data.startswith("cancelar_cita_confirmado:"):
        ...
        context.bot.send_message(chat_id=query.message.chat_id, text="La cancelación ha sido cancelada.")
